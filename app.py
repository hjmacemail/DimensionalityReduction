...
def _pdf_table_page(pdf, title, subtitle, text_df, highlight_row="Proposed"):
    """Render one DataFrame (string cells) as a full-page table in the PDF."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    n_rows = len(text_df.index)
    fig, ax = plt.subplots(figsize=(8.27, min(11.69, 1.6 + 0.42 * n_rows)), dpi=150)
    ax.axis("off")
    ...
    tbl = ax.table(
        cellText=[[str(v) for v in row] for row in text_df.to_numpy()],
        rowLabels=list(text_df.index), colLabels=list(text_df.columns),
    ...
    for (r, c), cell in tbl.get_cells().items() if hasattr(tbl, "get_cells") else tbl.get_celld().items():
        ...
        if r > 0 and list(text_df.index)[r - 1] == highlight_row:
...
def build_pdf_report(mean_df, std_df, text_df, per_ds, ds_names, sel_labels,
                     cfg_manifest, bullets, pareto, methods_used, selected_metrics,
                     axis_mean=None, axis_std=None):
    """Assemble a multi-page PDF report of the experiment results. Returns bytes."""
    import io
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    ...
    with PdfPages(buf) as pdf:
        ...
        fig.text(0.06, 0.94, "Causality-Aware Stable Hierarchical Feature Selection",
                 fontsize=15, fontweight="bold")
        fig.text(0.06, 0.915, "Experiment report", fontsize=11, color="#475569")
        import datetime as _dt
        lines = [
            f"Generated: {_dt.datetime.now():%Y-%m-%d %H:%M}",
            f"Datasets ({len(ds_names)}): {', '.join(ds_names)}",
            f"Methods: {', '.join(methods_used)}",
            f"k: {cfg_manifest.get('k')}   ·   bootstrap resamples: {cfg_manifest.get('bootstrap_resamples')}",
            f"Seeds: {cfg_manifest.get('seeds')}   ·   strict causal mode: {cfg_manifest.get('strict_causal_mode')}",
        ]
        fig.text(0.06, 0.87, "\n".join(lines), fontsize=9, va="top", color="#1e293b")
        fig.text(0.06, 0.75, "Best per metric", fontsize=11, fontweight="bold")
        clean_bullets = [b.replace("**", "") for b in bullets]
        fig.text(0.06, 0.725, "\n".join(f"• {b}" for b in clean_bullets),
                 fontsize=9, va="top")
        pstr = ", ".join(pareto)
        prop = "Proposed is Pareto-optimal (accuracy vs. stability)." if "Proposed" in pareto \
            else "Pareto-optimal (accuracy vs. stability): " + pstr
        fig.text(0.06, 0.63, "Trade-off", fontsize=11, fontweight="bold")
        fig.text(0.06, 0.605, prop, fontsize=9, va="top")
        fig.text(0.06, 0.04, "Generated by the Causal-HFS platform.", fontsize=7,
                 color="#94a3b8")
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)

        # ---- Aggregate metrics table ----
        scope = (f"Mean ± std across {len(ds_names)} datasets and "
                 f"{len(cfg_manifest.get('seeds', [0]))} seed(s). Runtime: lower is better.")
        _pdf_table_page(pdf, "Aggregate metrics", scope, text_df[sel_labels])

        # ---- Charts page ----
        try:
            fig = metric_bars(mean_df[sel_labels], std_df[sel_labels], sel_labels)
            fig.suptitle("Metric comparison (bars = mean · whiskers = std)",
            ...
            pass
        try:
            am = axis_mean if axis_mean is not None else mean_df
            asd = axis_std if axis_std is not None else std_df
            acc_l, stab_l = METRIC_LABELS["accuracy"], METRIC_LABELS["stability"]
            if len(methods_used) >= 2 and acc_l in am.columns and stab_l in am.columns:
                valid = am[[acc_l, stab_l]].dropna().index.tolist()
                if len(valid) >= 2:
                    fig = pareto_scatter(am.loc[valid], asd.loc[valid], acc_l, stab_l)
                    fig.suptitle("Accuracy vs. stability (green = Pareto-optimal)",
                                 fontsize=10, fontweight="bold")
                    pdf.savefig(fig, bbox_inches="tight")
                    plt.close(fig)
        except Exception:
        ...
        if len(ds_names) > 1:
            for d in ds_names:
                ...
                sub_txt = sub.copy()
                for _c in sub_txt.columns:
                    sub_txt[_c] = [f"{v:.3f}" if pd.notna(v) else "—" for v in sub[_c]]
                _pdf_table_page(pdf, f"Per-dataset — {d}", None, sub_txt)
...
def run_experiments(selected, k, n_bootstrap, methods, progress=None, strict_causal=False,
    ...
    for disp in selected:
        ...
        # High-dim budget guard: on wide data every method is refit many times
        # (consensus × stability bootstraps), each running an O(p²) Markov-Blanket
        # search. We shrink the univariate prefilter AND cap the resample budgets so
        # runs like Olivetti (1200-d) finish in ~1–2 min instead of stalling. Selection
        # is a bit noisier with a smaller candidate pool / fewer resamples, but it
        # completes; narrow datasets are unaffected.
        wide_dim = X.shape[1] > 200
        very_wide = X.shape[1] > 600
        pf_base = 80 if very_wide else (110 if wide_dim else 150)
        pf_top = max(pf_base, kk + 30)
        nb_eval = n_bootstrap if not wide_dim else 3
        nb_consensus = max(8, n_bootstrap) if not wide_dim else 3
        ...
        def prop_build_factory(seed):
            if strict_causal:
                ...
                return CausalHFS(FrameworkConfig(
                    n_representatives=kk, n_bootstrap=nb_consensus,
        ...
        for si in range(n_seeds):
            ...
            for m in methods:
                ...
                r = evaluate_method(m, bf, X, y, kk, n_bootstrap=nb_eval,
...
with st.sidebar:
    ...
    st.caption("Labels show rows×dims · (sklearn) offline · (UCI) & high-dim sets need "
               "internet · wide sets use a univariate prefilter (top-110 above 200 dims, "
               "top-80 above 600) and fewer resamples so they finish in ~1–2 min — keep "
               "methods/seeds modest for the very wide ones.")
...
with tab_exp:
    ...
    if df is None:
        ...
        with ex:
            ...
            st.markdown("**📄 PDF report** — a shareable summary: run configuration, "
                        "aggregate metrics, charts, and per-dataset tables.")
            try:
                pdf_bytes = build_pdf_report(
                    mean_full, std_full, text_full, per_ds, ds_names, sel_labels,
                    cfg_manifest, bullets, pareto, methods_used, selected_metrics,
                    axis_mean=axis_mean, axis_std=axis_std)
                st.download_button("⬇ PDF report", pdf_bytes, "causal_hfs_report.pdf",
                                   "application/pdf", use_container_width=True,
                                   type="primary")
            except Exception as exc:
                st.warning(f"Could not build the PDF ({exc}). The other formats below still work.")
